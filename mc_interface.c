...
static void update_override_limits(volatile motor_if_state_t *motor, volatile mc_configuration *conf) {
	...
	// float lo_battery_high_cut_start = 45.0;
	// float lo_battery_high_cut_end = 50.0;

	// // High Voltage Cutoff // by KiteX
	// float lo_in_min_batt = 0.0;
	// if (v_in < lo_battery_high_cut_start) {
	// 	lo_in_min_batt = conf->l_in_current_min;
	// } else if (v_in > lo_battery_high_cut_end) {
	// 	lo_in_min_batt = 0.0;
	// } else {
	// 	lo_in_min_batt = utils_map(v_in, lo_battery_high_cut_start,
	// 			lo_battery_high_cut_end, conf->l_in_current_min, 0.0);
	// }
